...
if(@$_REQUEST['category_name']!=''){
 $catnamere = "SELECT category_name  FROM  categories WHERE category_status!='-1' AND id!='".$_REQUEST['id']."' AND category_name='".mysql_real_escape_string($_REQUEST['category_name'])."'"; 
 $catnamere_num = sqlnumber($catnamere);
  }
 if(@$_REQUEST['category_code']!=''){
  $catcodere = "SELECT category_name  FROM  categories WHERE category_status!='-1' AND id!=".$_REQUEST['id']." AND category_code='".mysql_real_escape_string($_REQUEST['category_code'])."'"; 
 $catcodere_num = $db->getRow($catcodere);
 }
 if(@$_REQUEST['category_alias']!=''){
 $catalire = "SELECT category_name  FROM  categories WHERE category_status!='-1' AND id!=".$_REQUEST['id']." AND category_alias='".mysql_real_escape_string($_REQUEST['category_alias'])."' AND category_alias!=''"; 
