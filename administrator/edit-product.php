...
if(isset($_REQUEST['submit']) && $_REQUEST['submit']=='Save'){
  ...
  $productnamere = "SELECT product_name  FROM  products WHERE product_status!='-1' AND id!=".$_REQUEST['id']." AND product_name='".mysql_real_escape_string($_REQUEST['product_name'])."'"; 
  $productnamere_num = sqlnumber($productnamere);
  }
 if(@$_REQUEST['product_code']!=''){
 $productcodere = "SELECT product_code  FROM  products WHERE product_status!='-1' AND id!=".$_REQUEST['id']." AND product_code='".mysql_real_escape_string($_REQUEST['product_code'])."'"; 
 $productcodere_num = $db->getRow($productcodere);
 }
 if(@$_REQUEST['product_alias']!=''){
 $productalire = "SELECT product_alias  FROM  products WHERE product_status!='-1' AND id!=".$_REQUEST['id']." AND product_alias='".mysql_real_escape_string($_REQUEST['product_alias'])."' AND product_alias!=''"; 
...
<td width="50">&nbsp;</td>
<td width="650" align="left">
<input type="checkbox" style="margin-left:25px; margin-right:10px;"  name="product_status" id="product_status" value="1" <?php if($Productedit['product_status']==1){ ?> checked="checked"<?php } ?>/>Available
...
<td width="50">&nbsp;</td>
<td width="650" align="left"><input type="file" style="width:300px;"  name="product_primary_image" id="product_primary_image"/>
<br />
<br />
<img src="../<?php echo $Productedit['product_primary_image']; ?>"  style="width:225px;height:225px;"  /></td>
</tr>
<tr>
