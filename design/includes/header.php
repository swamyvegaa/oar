<!-- Top Area : Start -->
	<div id="top-area">

		<a href="#" id="top-area-trigger" class="top-area-closed"></a>

		<div class="top-area-wrap clearfix" align="right">
			
			<!--<p>iFortune .... <a href="#">Link</a></p>-->
			<div class="widget widget-social" align="right">
                    	<a title="facebook" href="#" class="soc_facebook">
					<img src="design/images/icons/facebook.png"></a>
<a title="google" href="#" class="soc_google">
					<img src="design/images/icons/google.png"></a>
<a title="twitter" href="#" class="soc_twitter">
					<img src="design/images/icons/twitter.png"></a>
<a title="youtube" href="#" class="soc_youtube">
					<img src="design/images/icons/youtube.png"></a>
<a title="vimeo" href="#" class="soc_vimeo">
					<img src="design/images/icons/vimeo.png"></a>
<a title="flickr" href="#" class="soc_flickr">
					<img src="design/images/icons/flickr.png"></a>
<a title="picasa" href="#" class="soc_picasa">
					<img src="design/images/icons/picasa.png"></a>
<a title="linkedin" href="#" class="soc_linkedin">
					<img src="design/images/icons/linkedin.png"></a>
<a title="dribbble" href="#" class="soc_dribbble">
					<img src="design/images/icons/dribbble.png"></a>
<a title="RSS" href="#" class="soc_rss">
					<img src="design/images/icons/rss.png"></a>
</div>
		</div>

	</div>
<!-- Top Area : End -->       

<!-- Top Bar : Start -->
	<div id="top-bar">
		
		<div class="col_half">
			<div id="top-menu">
				<!--<ul>
					<li><a href="index.php">Home</a></li>
					<li><a href="contact-us.php">Contact Us</a></li>
					<li><a href="archives.html">Archives</a></li>
					<li><a href="/blog">Blog</a></li>
					<li><a href="faqs.html">FAQs</a></li>
					<li><a href="sitemap.html">Sitemap</a></li>
                            
				</ul> -->
			</div>
		</div>
                
        <div class="col_half col_last">
			<div style="margin-top:10px; margin-right:45px; float:right;">	
			<a href="user-login.php">User Login</a>&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;&nbsp;  
			<a href="dealer-login.php">Dealer Login</a>
			<!--<a href="#">Designer Login</a> -->
			</div>
            
        </div>
            
	</div>
 <!-- Top Bar : End -->
 
<!-- Header Area : Start -->
 <div id="header" class="clearfix">

		<div id="logo">
			<a href="index.php"><img src="design/images/oar-logo-200x100.jpg" alt="On Antique Row" title="On Antique Row" /></a>
        </div>

	<div id="top-contacts-area">
    	<div class="header-form">
    		<form>
    			<input type="text" name="" id="product-search" onblur="if(this.value=='')this.value='Product Search'" onfocus="if(this.value=='Product Search')this.value=''" value="Product Search" />
        		<input type="button" name="" id="" value="GO" />
        		<input type="text" name="" id="Dealer-search" onblur="if(this.value=='')this.value='Dealer Search'" onfocus="if(this.value=='Dealer Search')this.value=''" value="Dealer Search" />
        		<input type="button" name="" id="" value="GO" />
        		<!--<input type="text" name="" id="Designer-search" onblur="if(this.value=='')this.value='Designer Search'" onfocus="if(this.value=='Designer Search')this.value=''" value="Designer Search" />
        		<input type="button" name="" id="" value="GO" /> -->
    		</form>
            <div class="clear"></div>
            <!--<ul id="top-contacts">
				<li><span>Contact</span><strong>+91 800-8888-800</strong></li>
			<li><span>Fax</span>+03.2312.4442</li>
			<li><span>Email</span>info@ifortune.org</li>
			</ul>-->
    	</div>         
                    
                
	</div>
</div>
<!-- Header Area : End -->

